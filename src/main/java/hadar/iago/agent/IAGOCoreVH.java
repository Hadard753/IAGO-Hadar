...
public abstract class IAGOCoreVH extends GeneralVH
{
	...
	private Ledger myLedger = new Ledger();
	 // The things we wants to remember between rounds                                 private int currentGameCount = 0;
    private int greedyMeter = 0;
    private int honestMeter = 0;
    // The things we want to remember within a round
    private Map<String, Integer> itemsRequested= new HashMap<String, Integer>();
    private Map<String, Integer> itemsGiven = new HashMap<String, Integer>();
    private Map<String, Integer> userDeclaredPriorities = new HashMap<String, Integer>();
    ...
    /**
     * Agents work by responding to various events. This method describes how core
     * agents go about selecting their responses.
     *
    ...
    public LinkedList<Event> getEventResponse(Event e) {
     LinkedList<Event> resp = new LinkedList<Event>();
        ...
        if (e.getType().equals(Event.EventClass.GAME_START)) {
            currentGameCount++;
            ServletUtils.log("Game number is now " + currentGameCount + "... reconfiguring!",
                    ServletUtils.DebugLevels.DEBUG);
            AgentUtilsExtension aue = new AgentUtilsExtension(this);
            aue.configureGame(game);
            this.utils = aue;
            this.messages.setUtils(utils);
            ...
            // if we wanted, we could change our Policies between games... but not easily.
            // Probably don't do that. Just don't.
            ...
            setItemsRequested(new HashMap<String, Integer>());
            setItemsGiven(new HashMap<String, Integer>());
            setUserDeclaredPriorities(new HashMap<String, Integer>());
            ...
            if (!firstGame) {
                ...
                if (greedyMeter >= 2) {
                    newGameMessage = "And so we meet again ;)\n let try to corporate better this time ah?";
                } else {
                    newGameMessage = "It's good to see you again!  Let's get ready to negotiate again";
                }
                Event e0 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.NONE, newGameMessage,
                (int) (100 * game.getMultiplier()));
                resp.add(e0);
                if (lastOfferReceived != null) {
                    String oweUsMessage = "Remember... you owe us from the last round";
                    e0 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.NONE, oweUsMessage,
                    (int) (100 * game.getMultiplier()));
                    resp.add(e0);
                }
                return resp;
            }
            String newGameMessage = "Hello nice to meet you ! we can't wait to negotiate with you and have fun\n Remember, negotiation always have a win win option :)";
            Event e0 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.NONE, newGameMessage,
                    (int) (100 * game.getMultiplier()));
            resp.add(e0);
            firstGame = false;
        ...
        if (!firstFlag && !this.disable) {
            ...
            if (e2.getOffer() != null) {
                ...
                resp.add(e3);
                Event e4 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.NONE,
                        messages.getProposalLangFirst(), (int) (1000 * game.getMultiplier()));
                resp.add(e4);
            ...
            String revealBATNA = "Just so you know, I already have an offer for " + utils.myPresentedBATNA
                    + " points, so I won't accept anything less.   What about you?";
            Event e5 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.BATNA_INFO,
                    utils.myPresentedBATNA, revealBATNA, (int) (1000 * game.getMultiplier()));
            resp.add(e5);
            disable = false;

            Event e6 = messages.getFavorBehavior(getHistory(), game, e);

            if (e6 != null && (e6.getType() == EventClass.OFFER_IN_PROGRESS
                    || e6.getSubClass() == Event.SubClass.FAVOR_ACCEPT)) {
                Event e7 = new Event(this.getID(), Event.EventClass.SEND_OFFER, behavior.getNextOffer(getHistory()),
                        (int) (700 * game.getMultiplier()));
                if (e7.getOffer() != null) {
                    String s1 = messages.getProposalLang(getHistory(), game);
                    Event e8 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_PROPOSE, s1,
        ...
        // what to do when player sends an expression -- react to it with text and our
        // own expression
        if (e.getType().equals(Event.EventClass.SEND_EXPRESSION)) {
            String expr = expression.getExpression(getHistory());
            if (expr != null) {
                Event e1 = new Event(this.getID(), Event.EventClass.SEND_EXPRESSION, expr, 2000,
                        (int) (700 * game.getMultiplier()));
                resp.add(e1);
            }
            Event e0 = messages.getVerboseMessageResponse(getHistory(), game, e);
            if (e0 != null && (e0.getType() == EventClass.OFFER_IN_PROGRESS
                    || e0.getSubClass() == Event.SubClass.FAVOR_ACCEPT)) {
                Event e2 = new Event(this.getID(), Event.EventClass.SEND_OFFER, behavior.getNextOffer(getHistory()),
                        (int) (700 * game.getMultiplier()));
                if (e2.getOffer() != null) {
                    String s1 = messages.getProposalLang(getHistory(), game);
                    Event e1 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_PROPOSE, s1,
                            (int) (2000 * game.getMultiplier()));
                    resp.add(e0);
                    resp.add(e1);
                    resp.add(e2);
        ...
        if (e.getType().equals(Event.EventClass.FORMAL_ACCEPT)) {
            ...
            if (lastOffer != null && lastTime != null) {
                ...
                // down to the wire, accept anything better than BATNA (less than 30 seconds
                // from finishing time)
               ...
               if (utils.myActualOfferValue(lastOffer.getOffer()) > fairSplit - behavior.getAcceptMargin()) // accept                                                                                 // margin
                {
                    Event e0 = new Event(this.getID(), Event.EventClass.FORMAL_ACCEPT, 0);
                    resp.add(e0);
                    return resp;
                } else {
                    Event e1 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_REJECT,
                            messages.getRejectLang(getHistory(), game), (int) (700 * game.getMultiplier()));
                    resp.add(e1);
        ...
        if (e.getType().equals(Event.EventClass.TIME)) {
            noResponse += 1;
            for (int i = getHistory().getHistory().size() - 1; i > 0 && i > getHistory().getHistory().size() - 6; i--)// if
                                                                                                                      // something
                                                                                                                      // from
                                                                                                                      // anyone
                                                                                                                      // for
            ...
            if (noResponse >= 2) {
                Event e0 = messages.getVerboseMessageResponse(getHistory(), game, e);
                // Theoretically, this block isn't necessary. Event e0 should just be a message
                // returned by getWaitingLang
                if (e0 != null && (e0.getType() == EventClass.OFFER_IN_PROGRESS
                        || e0.getSubClass() == Event.SubClass.FAVOR_ACCEPT)) {
                    Event e2 = new Event(this.getID(), Event.EventClass.SEND_OFFER, behavior.getNextOffer(getHistory()),
                            (int) (700 * game.getMultiplier()));
                    if (e2.getOffer() != null) {
                        String s1 = messages.getProposalLang(getHistory(), game);
                        Event e1 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_PROPOSE,
                                s1, (int) (2000 * game.getMultiplier()));
                        resp.add(e0);
                        resp.add(e1);
                        resp.add(e2);
                        this.lastOfferSent = e2.getOffer();
                ...
                noResponseFlag = false;
                Event e2 = new Event(this.getID(), Event.EventClass.SEND_OFFER, behavior.getTimingOffer(getHistory()),
                        0);
                if (e2.getOffer() != null) {
                    Event e3 = new Event(this.getID(), Event.EventClass.OFFER_IN_PROGRESS, 0);
                    resp.add(e3);
                    Event e4 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_PROPOSE,
                            messages.getProposalLang(getHistory(), game), (int) (1000 * game.getMultiplier()));
                    resp.add(e4);
                    resp.add(e2);
                }
            }
            ...
            if (!timeFlag && game.getTotalTime() - Integer.parseInt(e.getMessage()) < 30) {
                timeFlag = true;
                Event e1 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.TIMING,
                        messages.getEndOfTimeResponse(), (int) (700 * game.getMultiplier()));
                resp.add(e1);
                Event e2 = new Event(this.getID(), Event.EventClass.SEND_OFFER, behavior.getFinalOffer(getHistory()),
                        0);
                if (e2.getOffer() != null) {
                    resp.add(e2);
                    lastOfferSent = e2.getOffer();
            ...
            // At 90 second, computer agent will send prompt for user to talk about
            // preferences
            if (e.getMessage().equals("90") && this.getID() == History.OPPONENT_ID) {
                ...
                Event e1 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.PREF_REQUEST, str,
        ...
        if (e.getType().equals(Event.EventClass.SEND_OFFER)) {
            ...
            ServletUtils.log("Allocated Agent Value: " + utils.myActualOfferValue(allocated),
                    ServletUtils.DebugLevels.DEBUG);
            ServletUtils.log("Conceded Agent Value: " + utils.myActualOfferValue(conceded),
                    ServletUtils.DebugLevels.DEBUG);
            ServletUtils.log("Offered Agent Value: " + utils.myActualOfferValue(o), ServletUtils.DebugLevels.DEBUG);
            int playerDiff = (utils.adversaryValue(o, utils.getMinimaxOrdering())
                    - utils.adversaryValue(allocated, utils.getMinimaxOrdering()));
            ServletUtils.log("Player Difference: " + playerDiff, ServletUtils.DebugLevels.DEBUG);

            if (utils.myActualOfferValue(o) >= utils.myActualOfferValue(allocated)) {// net positive (o is a better
                                                                                     // offer than allocated)
                int myValue = utils.myActualOfferValue(o) - utils.myActualOfferValue(allocated)
                        + behavior.getAcceptMargin();
                ServletUtils.log("My target: " + myValue, ServletUtils.DebugLevels.DEBUG);
                int opponentValue = utils.adversaryValue(o, utils.getMinimaxOrdering())
                        - utils.adversaryValue(allocated, utils.getMinimaxOrdering());
                if (myValue > opponentValue)
                    localFair = true;// offer improvement is within one max value item of the same for me and my
                                     // opponent
                else if (myValue == opponentValue && !firstOffer)
                    localEqual = true;
            }

            if (utils.myActualOfferValue(o) + behavior.getAcceptMargin() > utils.adversaryValue(o,
                    utils.getMinimaxOrdering()))
            ...
            if (localFair && !totalFair) {
                ...
                if (expr != null) {
                    Event eExpr = new Event(this.getID(), Event.EventClass.SEND_EXPRESSION, expr, 2000,
                            (int) (700 * game.getMultiplier()));
                    resp.add(eExpr);
                }
                Event e0 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_REJECT,
                        messages.getSemiFairResponse(), (int) (700 * game.getMultiplier()));
                resp.add(e0);
                behavior.updateAdverseEvents(1);
                Event e3 = new Event(this.getID(), Event.EventClass.SEND_OFFER, behavior.getNextOffer(getHistory()),
                        (int) (700 * game.getMultiplier()));
                if (e3.getOffer() != null) {
                    Event e1 = new Event(this.getID(), Event.EventClass.OFFER_IN_PROGRESS, 0);
                    resp.add(e1);
                    Event e2 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_PROPOSE,
                            messages.getProposalLang(getHistory(), game), (int) (3000 * game.getMultiplier()));
                    resp.add(e2);
                ...
                Event e0 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_ACCEPT,
                        messages.getVHAcceptLang(getHistory(), game), (int) (700 * game.getMultiplier()));
                ...
                Event e0 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_REJECT,
                        messages.getVHRejectLang(getHistory(), game), (int) (700 * game.getMultiplier()));
                // slightly alter the language
                if (localEqual)
                    e0 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_REJECT,
                            messages.getVHEqualLang(), (int) (700 * game.getMultiplier()));
                resp.add(e0);
                behavior.updateAdverseEvents(1);
                Event e3 = new Event(this.getID(), Event.EventClass.SEND_OFFER, behavior.getNextOffer(getHistory()),
                        (int) (700 * game.getMultiplier()));
        ...
        // what to do when the player sends a message (including offer acceptances and
        // rejections)
        if (e.getType().equals(Event.EventClass.SEND_MESSAGE)) {
            ...
            if (p != null && !p.isQuery()) // a preference was expressed
            {
                ...
                if (utils.reconcileContradictions()) {
                    // we simply drop the oldest expressed preference until we are reconciled. This
                    // is not the best method, as it may not be the the most efficient route.
            ...
            String expr = expression.getExpression(getHistory());
            if (expr != null) {
                Event e0 = new Event(this.getID(), Event.EventClass.SEND_EXPRESSION, expr, 2000,
                        (int) (700 * game.getMultiplier()));
                resp.add(e0);
            }
            ...
            boolean offerRequested = (e.getSubClass() == Event.SubClass.OFFER_REQUEST_NEG
                    || e.getSubClass() == Event.SubClass.OFFER_REQUEST_POS);

            if (offerRequested) {
                if (utils.adversaryBATNA + utils.myPresentedBATNA > utils.getMaxPossiblePoints()) {
                    String walkAway = "Actually, I won't be able to offer you anything that gives you "
                            + utils.adversaryBATNA + " points. I think I'm going to have to walk "
                            + "away, unless you were lying.";
                    Event e2 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.THREAT_NEG,
                            utils.adversaryBATNA, walkAway, (int) (2000 * game.getMultiplier()));
                    resp.add(e2);
                } else {
                    Event e2 = new Event(this.getID(), Event.EventClass.SEND_OFFER, behavior.getNextOffer(getHistory()),
                            (int) (3000 * game.getMultiplier()));
                    if (e2.getOffer() == null) {
                        String response = "Actually, I'm having trouble finding an offer that works for both of us.";
                        Event e4 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.THREAT_POS,
                                response, (int) (2000 * game.getMultiplier()));
                        if (utils.adversaryBATNA != -1) {
                            response += " Are you sure you can't accept anything less than " + utils.adversaryBATNA
                                    + " points?";
                            e4 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.BATNA_REQUEST,
                                    utils.adversaryBATNA, response, (int) (2000 * game.getMultiplier()));
                        }
                        resp.add(e4);
            ...
            if (e.getSubClass() == Event.SubClass.OFFER_ACCEPT)// offer accepted
            {
                ...
                Event e2 = new Event(this.getID(), Event.EventClass.SEND_OFFER,
                        behavior.getAcceptOfferFollowup(getHistory()), (int) (3000 * game.getMultiplier()));
                if (e2.getOffer() != null) {
                    Event e3 = new Event(this.getID(), Event.EventClass.OFFER_IN_PROGRESS, 0);
                    resp.add(e3);
                    Event e4 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_PROPOSE,
            ...
            if (e.getSubClass() == Event.SubClass.OFFER_REJECT)// offer rejected
            {
                ...
                Event e2 = new Event(this.getID(), Event.EventClass.SEND_OFFER,
                        behavior.getRejectOfferFollowup(getHistory()), (int) (3000 * game.getMultiplier()));
                if (e2.getOffer() != null) {
                    Event e3 = new Event(this.getID(), Event.EventClass.OFFER_IN_PROGRESS, 0);
                    resp.add(e3);
                    Event e4 = new Event(this.getID(), Event.EventClass.SEND_MESSAGE, Event.SubClass.OFFER_PROPOSE,
	...
	public Map<String, Integer> getUserDeclaredPriorities() {
		return userDeclaredPriorities;
	}

	public void setUserDeclaredPriorities(Map<String, Integer> userDeclaredPriorities) {
		this.userDeclaredPriorities = userDeclaredPriorities;
	}

	public Map<String, Integer> getItemsGiven() {
		return itemsGiven;
	}

	public void setItemsGiven(Map<String, Integer> itemsGiven) {
		this.itemsGiven = itemsGiven;
	}

	public Map<String, Integer> getItemsRequested() {
		return itemsRequested;
	}

	public void setItemsRequested(Map<String, Integer> itemsRequested) {
		this.itemsRequested = itemsRequested;
	}

	public int getHonestMeter() {
		return honestMeter;
	}

	public void setHonestMeter(int honestMeter) {
		this.honestMeter = honestMeter;
	}
