...
import hadar.iago.agent.RepeatedFavorBehavior.LedgerBehavior;
...
public class RepeatedFavorMessage extends IAGOCoreMessage implements MessagePolicy {
	protected final String[] proposal = {"I think this deal is good for the both of us.",
			"I think this offer is the exact win win situation I was talking about",
			"I think this arrangement is fair.",
			"I think this deal will interest you, next offer you can pay me back",
	...
	protected final String[] rejectResponse = {
			"Oh that's too bad, what part of this offer would you agree to accept?",
			"Ah well, we will find a better offer that works for both of us",
			"Ok, maybe I misunderstood your preferences, would you kindly correct me ?",
	"Alright."};

	protected final String[] vhReject = {
			"I'm sorry, but I don't think that's fair to me, let's find a win win",
			"Apologies, but that won't work for me, but Im sure we can figure something else out",
			"Perhaps we should spend more time finding a solution that's good for us both...",
	"I won't be able to accept that. let try another offer"};

	protected final String[] vhAccept = {
			"Your offer is good, this is a win win!",
			"That seems like a good deal :)",
			"That will work for us",
	"That's not quite what I hoped for but for you I will do it !"};
	...
	protected final String[] vhWaiting = {
		...
		"Maybe we can discuss a little more on our priorities",
		"We should try harder to find a deal that benefits us both.", null};

	private String[] vhIdleQuestions = {
		"Do you like something best?",
		"So could you tell me about your preferences?"};
		//P++ will only use each of these messages once at the game's start before trying other messages
	...
	private String getEmotionResponse(History history, GameSpec game, Event e) {
		...
		if(e.getMessage().equals("sad") || (e.getMessage().equals("angry")))
			return "Oh, we didn't meant to upset you, let try a different direction";
		else if(e.getMessage().equals("happy"))
			return "We are both happy then ! :)";
	...
	protected String getEndOfTimeResponse() {
		return "This is good of us ! Let do it";
	}

	protected String getSemiFairResponse() {
		return "So close, let make it a bit sweeter :)";
	}
	...
	{
		// INFO: the basic agent would requested for favors under certain conditions but we are trying to build trust with our 
		// opponent and therefore we won't innitiate any favor request but only redeem them if he owe us
		if (lb != LedgerBehavior.NONE && utils.getLedger() > 0)
			return new Event(agentID, Event.EventClass.SEND_MESSAGE, Event.SubClass.FAVOR_REQUEST,
					"We were happy to give you a favor when you needed it! will you return the favor ?",
		...
		else if (lb != LedgerBehavior.NONE && lb != LedgerBehavior.BETRAYING && utils.getLedger() < 0)
		{
			...
			return new Event(agentID, Event.EventClass.SEND_MESSAGE, Event.SubClass.FAVOR_ACCEPT,
					"Thanks for giving us a favor when we needed it!  Let me return the favor",
					(int) (1000*game.getMultiplier()));
		}
		return null;
	}
	...
	public Event getVerboseMessageResponse(History history, GameSpec game, Event ePrime) {
		...
		case OFFER_REQUEST_NEG:
			str = "Alright, let do it , it a win win !";
			sc = Event.SubClass.OFFER_PROPOSE;
			break;
		...
		case FAVOR_REQUEST:
			if(utils.isImportantGame())
			{
				if (utils.getLedger() < 0)
					str += "I know I owe you, and I will return it to you next round! but items this game are worth so much to me...";
				else
			...
			{
				// TODO: Check if that not the last round
			...
			{
				str += "I'm sorry but you still owe me from before...";
				sc = Event.SubClass.FAVOR_REJECT;	
			}			
			break;
		case FAVOR_RETURN:
